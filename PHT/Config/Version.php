<?php
/**
 * PHT
 *
 * @author Telesphore
 * @link https://github.com/jetwitaussi/PHT
 * @version 3.0
 * @license "THE BEER-WARE LICENSE" (Revision 42):
 *          Telesphore wrote this file.  As long as you retain this notice you
 *          can do whatever you want with this stuff. If we meet some day, and you think
 *          this stuff is worth it, you can buy me a beer in return.
 */

namespace PHT\Config;

class Version
{
    const ACHIEVEMENTS = '1.1';
    const ALLIANCEDETAILS = '1.5';
    const ALLIANCES = '1.4';
    const ARENADETAILS = '1.5';
    const AVATARS = '1.1';
    const BOOKMARKS = '1.0';
    const CHALLENGES = '1.5';
    const CLUB = '1.5';
    const CUPMATCHES = '1.4';
    const CURRENTBIDS = '1.0';
    const ECONOMY = '1.3';
    const FANS = '1.2';
    const HOFPLAYERS = '1.1';
    const LADDERDETAILS = '1.0';
    const LADDERLIST = '1.0';
    const LEAGUEDETAILS = '1.4';
    const LEAGUEFIXTURES = '1.2';
    const LIVE = '1.8';
    const MANAGERCOMPENDIUM = '1.0';
    const MATCHDETAILS = '2.7';
    const MATCHES = '2.8';
    const MATCHESARCHIVE = '1.3';
    const MATCHLINEUP = '1.9';
    const MATCHORDERS = '2.4';
    const NATIONALPLAYERS = '1.3';
    const NATIONALTEAMDETAILS = '1.8';
    const NATIONALTEAMMATCHES = '1.3';
    const NATIONALTEAMS = '1.5';
    const PLAYERDETAILS = '2.6';
    const PLAYEREVENTS = '1.3';
    const PLAYERS = '2.3';
    const REGIONDETAILS = '1.2';
    const SEARCH = '1.2';
    const STAFFAVATARS = '1.0';
    const STAFFLIST = '1.1';
    const SUPPORTERS = '1.0';
    const TEAMDETAILS = '3.2';
    const TOURNAMENTDETAILS = '1.0';
    const TOURNAMENTFIXTURES = '1.0';
    const TOURNAMENTLEAGUETABLES = '1.0';
    const TOURNAMENTLIST = '1.0';
    const TRAINING = '2.2';
    const TRAININGEVENTS = '1.3';
    const TRANSFERSEARCH = '1.0';
    const TRANSFERSPLAYER = '1.1';
    const TRANSFERSTEAM = '1.2';
    const TRANSLATIONS = '1.1';
    const WORLDCUP = '1.1';
    const WORLDDETAILS = '1.6';
    const WORLDLANGUAGES = '1.2';
    const YOUTHAVATARS = '1.1';
    const YOUTHLEAGUEDETAILS = '1.0';
    const YOUTHLEAGUEFIXTURES = '1.0';
    const YOUTHPLAYERLIST = '1.1';
    const YOUTHPLAYERDETAILS = '1.1';
    const YOUTHTEAMDETAILS = '1.1';
}
