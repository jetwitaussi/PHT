...
class Youth extends Xml\File
{
     ...
     * @return integer
     */
    public function getLastPlayedRoundNumber()
    {
    ...
    {
        return $this->getXml()->getElementsByTagName('NrOfTeamsInLeague')->item(0)->nodeValue;
    }
    ...
    public function getRound($id)
    {
        ...
        if ($id >= Config\Config::$forIndex && $id < (($this->getTeamsNumber() - 1) * 2) + Config\Config::$forIndex) {
            ...
            return new Xml\World\League\Season\Round($matches, Config\Config::YOUTH);
        }
        return null;
    }
    ...
    public function getNextRound()
    {
        foreach ($this->getRounds() as $round) {
            if ($round->getMatch(Config\Config::$forIndex)->getStatus() == Config\Config::MATCH_TOCOME) {
                return $round;
            }
        }
        return null;
    ...
    {
        $rounds = array();
        for ($i = Config\Config::$forIndex; $i < (($this->getTeamsNumber() - 1) * 2) + Config\Config::$forIndex; $i++) {
    ...
    public function getStandings()
    {
        ...
        foreach ($tmp as $id => $data) {
            ...
            $list[$value . '-' . $data['name']] = new Xml\World\League\Season\Team($id, $data, Config\Config::YOUTH);
        }
        krsort($list);
        $teams = array();
